...
const Edit = () => {
    ...
    const [editoy, seteditoy] = useState()
    const { id } = useParams()
    useEffect(() => {
        fetch(`http://localhost:5000/addtoy/${id}`)
            .then(res => res.json())
            .then(toy => seteditoy(toy))
    }, [id])
    const { name, sellerName, price, rating, quantity, photo, catagory, discription } = editoy || {}
    ...
    const hendelAddToy = event => {
        ...
        fetch(`https://toy-cor-server.vercel.app/deleteDb/${id}`, {
            method: 'PUT',
            headers: {
            ...
            toast('Success the edite prodack')
    }
    return (
        <div>
                        ...
                        <form onSubmit={hendelAddToy} className=" px-10 py-4">
                            <div className=' grid grid-cols-2 gap-4'>
                                ...
                                <div>
                                    ...
                                    <input defaultValue={sellerName} type="text" name='sellername' placeholder="seller name" className="input input-bordered w-full" />
                                </div>
                                <div>
                                    <label className="label">
                                        <span className="label-text">Seller Email</span>
                                    </label>
                                    <input type="email" name='email' placeholder="seller email" defaultValue={user.email} className="input input-bordered w-full" readOnly />
                                </div>
                                <div>
                                    <label className="label">
                                ...
                                <div>
                                    ...
                                    <select defaultValue={catagory} name="catagory" className="input input-bordered w-full" >
                            ...
                            <div>
                                ...
                                <textarea defaultValue={discription} name="discription" cols="30" rows="5" placeholder='Please Your Discription' className="input pt-2 input-bordered w-full border h-full"></textarea>
                            </div>
